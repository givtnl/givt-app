...
class BarcodeHuntLevelsPage extends StatelessWidget {
  const BarcodeHuntLevelsPage({super.key});
      ...
      onCustom: (context, custom) {
        if (custom is NavigateToLevelIntroduction) {
          Navigator.of(context).push(
            const BarcodeHuntLevelIntroductionPage().toRoute(context),
          );
        }
      },
    );
  ...
  Widget build(BuildContext context) {
    return FunMissionCard(
      uiModel: FunMissionCardUIModel(
        ...
        headerIcon: !unlocked ? FunIcon.lock() : null,
        progress: unlocked
