...
class RecurringDonationsOverviewRepositoryImpl with RecurringDonationsOverviewRepository {
  ...
  final StreamController<List<RecurringDonation>> _donationsController = StreamController<List<RecurringDonation>>.broadcast();
  ...
  Future<void> loadRecurringDonations() async {
    ...
    try {
      final response = await apiService.fetchRecurringDonations(params: {'userId': 'current'});
      ...
      for (var i = 0; i < result.length; i++) {
        ...
        final collectGroup = collectGroups.firstWhere(
          ...
          orElse: () {
            ...
            print('Collection with namespace ${recurringDonation.namespace} not found');
            return const CollectGroup.empty();
          },
        );

        final recurringDonationWithCollectGroup = 
