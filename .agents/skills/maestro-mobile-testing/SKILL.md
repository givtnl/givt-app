...
## Givt app conventions

This repository is **Flutter**, not React Native. Project-specific layout and patterns live in **`test/Maestro/README.md`** — read that before adding or moving flows.

### Quick reference

| Topic | Givt approach |
|-------|----------------|
| Test location | `test/Maestro/{EU\|US}/` |
| Selectors | `Semantics(identifier: …)` / `semanticsIdentifier` on FUN widgets — use Maestro `id:` |
| App ID | `net.givtapp.ios.test` (development flavor) |
| Shared subflows | `EU/_shared/` (e.g. `Login_AuthNeeded.yml`, `Dismiss_BottomSheet.yml`) |
| Feature folders | e.g. `EU/Account_Settings/` when a cluster has several related flows |
| E2E orchestrators | Region root (`E2E_NL_NewUser_DoDonation.yml`) when spanning multiple features |
| Re-auth | Conditional `runFlow` when `Login-Bottomsheet-Password-Input` is visible — see README |

### Flutter semantics (not testID)

```dart
// FUN components — prefer semanticsIdentifier parameter
FunButton(
...
```yaml
# Maestro — same as testID syntax
- tapOn:
    id: "Email-Continue-Button"
```

### Conditional re-auth (required pattern)
...
Do **not** use `optional: true` on a flow that taps visible text like `"Password"` — localized copy and duplicate labels cause flaky or wrong taps.

### Run (Givt)

```bash
maestro test test/Maestro/EU/Account_Settings/
maestro test --debug test/Maestro/EU/Account_Settings/E2E_NL_Login.yml
```

Default NL test account: `donna+maestronl@givtapp.net` / `Test123` (set in login subflow `env`).
