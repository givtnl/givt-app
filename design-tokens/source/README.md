...
This generates `fun_givt_tokens_generated.dart` and `fun_givt4kids_tokens_generated.dart` under `lib/shared/design_system/tokens/`. The app uses the hand-written `FunGivtTokens` and `FunGivt4KidsTokens` (which implement `FunThemeTokens`); the generated files can be wired in or used as reference.
